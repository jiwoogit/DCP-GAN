...
def Get_Network_Shape(model_dict):
    ...
    '''
    conv_key_list = model_dict.synthesis.layer_names
    num_channels = [getattr(model_dict.synthesis, key).in_channels for key in conv_key_list] # from start to end num_channels.append(model_dict[conv_key_list[-1]].shape[1])        # last layer
    return num_channels
