...
def Get_Diversity_Pruning_Score(generator, n_sample, batch_size, device, \
    edit_strength=5.0, n_direction=10, truncation_trick=1.0, noise_path=None):
    ...
    latents = generator.style(noise_z)
    mean_w = torch.mean(latents, dim=0).detach().clone()

    # get pca latent directions
    ...
    for idx, batch in enumerate(tqdm(batch_size_list)):
        ...
        for i in range(n_components):
            ...
            latents_pca = latents + (direction * alpha)
            generator.requires_grad = True
            img = generator(None, input_is_latent=True, latent_styles=[latents], truncation=truncation_trick, truncation_latent=mean_w)

            # stop grad
            generator.requires_grad = False
            img_pca = generator(None, input_is_latent=True, latent_styles=[latents_pca], truncation=truncation_trick, truncation_latent=mean_w)

            grad_score_1 = Get_Weight_Gradient(img_pca, img, generator)
            generator.zero_grad()
